...
def validate_materialized_manifest(path: str | Path) -> dict:
    ...
    the training/evaluation distribution.
    """
    ...
    for row in rows:
        ...
        if not local_path or not Path(local_path).is_file():
            missing.append(str(row.get("sample_id") or "(unknown)"))
        if len(sha256) != 64 or any(ch not in "0123456789abcdefABCDEF" for ch in sha256):
            bad_hash.append(str(row.get("sample_id") or "(unknown)"))
    ...
    if missing or bad_hash or unbalanced:
        problems = []
        if missing:
...
def extract_verified_materialized_embeddings(*args, manifest_csv, **kwargs):
    validation = validate_materialized_manifest(manifest_csv)
    result = extract_materialized_embeddings(manifest_csv, *args, **kwargs)
