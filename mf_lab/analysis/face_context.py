...
def face_context_consistency(path: str | Path) -> dict:
    """Compare face-region texture/noise statistics with nearby context."""
    ...
    for x, y, w, h in faces[:10]:
        ...
        face_std = float(np.std(face_res)); bg_std = float(np.std(bg_res))
        noise_ratio = face_std / (bg_std + 1e-12)
        face_lap = float(cv2.Laplacian(face, cv2.CV_32F).var())
        bg_img = np.zeros_like(ctx); bg_img[mask] = ctx[mask]
        ...
        rows.append({
            ...
            "face_residual_std": face_std, "context_residual_std": bg_std,
            "face_context_noise_ratio": float(noise_ratio), "face_context_sharpness_ratio": float(sharp_ratio),
            "screening_flags": flags,
        })
    flags = sorted({f for r in rows for f in r["screening_flags"]})
    return {
        "status": "screening_only", "faces_detected": int(len(faces)), "faces_analyzed": len(rows),
        "face_context_metrics": rows, "screening_flags": flags, "calibrated": False,
