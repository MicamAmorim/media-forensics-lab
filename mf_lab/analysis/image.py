...
def jpeg_dct_periodicity(path: str | Path) -> dict:
    ...
    return {"score": score, "n_blocks": int(q.size), "zero_bin_fraction": z, "status": "screening_only",
            "warning": "Heuristic indicator; cite/validate a published detector before evidentiary use."}


def copy_move_orb(path: str | Path, descriptor_distance: int = 55, spatial_distance: float = 40.0,
                  displacement_bin_px: float = 8.0, min_cluster_matches: int = 10,
                  max_cluster_mean_hamming: float = 40.0, max_cluster_displacement_std: float = 1.2) -> dict:
    ...
    if des is None or len(kp) < 4:
        return {"keypoints": len(kp), "candidate_pairs": 0, "suspicious_pairs": 0,
                "translation_clusters": [], "score": 0.0, "status": "screening_only"}
    ...
    for query_idx, matches in enumerate(knn):
        for mt in matches:
            ...
            if mt.distance <= descriptor_distance and separation >= spatial_distance:
                ...
                candidates.append({"query_idx": int(query_idx), "train_idx": int(mt.trainIdx),
                                   "hamming": float(mt.distance), "separation": separation,
                                   "p1": p1, "p2": p2, "displacement": displacement})
                break
    ...
    for rows in bins.values():
        ...
        suspicious = (len(rows) >= min_cluster_matches and mean_hamming <= max_cluster_mean_hamming and disp_std <= max_cluster_displacement_std)
        clusters.append({"matches": len(rows), "mean_hamming": mean_hamming,
                         ...
                         "source_bbox": [float(p1[:,0].min()), float(p1[:,1].min()), float(p1[:,0].max()), float(p1[:,1].max())],
                         "destination_bbox": [float(p2[:,0].min()), float(p2[:,1].min()), float(p2[:,0].max()), float(p2[:,1].max())],
    ...
    return {"keypoints": len(kp), "candidate_pairs": len(candidates), "suspicious_pairs": suspicious_pairs,
            "suspicious_cluster_count": len(suspicious_clusters),
            "dominant_translation_px": dominant["translation_px"] if dominant else None,
            "translation_clusters": clusters[:25], "score": score, "status": "screening_only",
            "thresholds": {"descriptor_distance": descriptor_distance, "spatial_distance": spatial_distance,
                           "displacement_bin_px": displacement_bin_px, "min_cluster_matches": min_cluster_matches,
