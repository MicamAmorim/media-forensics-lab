...
def _cant_split_row(row) -> None:
    ...
    if trPr.find(qn := "{http://schemas.openxmlformats.org/wordprocessingml/2006/main}cantSplit") is None:
...
def _answer_question(q: str, reports: list[dict], assessment: str) -> str:
    ...
    if "metadad" in ql or "estrutura" in ql or "origem" in ql:
        return (
            "O laboratório registra e descreve metadados, estrutura e proveniência disponível, mas a compatibilidade com uma narrativa de origem "
            "depende de informação externa documentada e, quando possível, confronto com o arquivo/dispositivo fonte. Metadados ausentes ou editáveis não bastam, isoladamente, para confirmar ou negar a narrativa."
        )
    if "sintét" in ql or "ia" in ql or "deepfake" in ql:
        ...
        return (
            f"Foram executados {len(protos)} protocolo(s) MFLAB-DF; {review} indicou(aram) necessidade de revisão aprofundada e "
            f"{validated} saída(s) de modelo externo foram explicitamente marcadas como validadas no caso. O pipeline não converte heurísticas ou um único escore em certeza. "
            "Assim, salvo convergência adicional documentada por especialista, o resultado automático permanece inconclusivo quanto à geração sintética/IA."
        )
...
def _short_result(method: str, value) -> str:
    ...
    if method in {"deepfake_protocol", "video_deepfake_protocol"}:
        ...
        return json.dumps(compact, ensure_ascii=False)[:900]
    if method == "metadata":
    ...
    if method == "c2pa":
        compact = {k: value.get(k) for k in ("status", "tool", "reason") if value.get(k) is not None}
        if value.get("status") == "success": compact["manifest_present"] = True
        return json.dumps(compact, ensure_ascii=False)
    if method == "histogram":
        compact = {
    ...
    if method == "frequency":
        compact = {k: value.get(k) for k in ("high_low_frequency_ratio", "spectral_peak_count", "spectral_peak_indices", "quadrant_mean_cv")}
        return json.dumps(compact, ensure_ascii=False)
...
def generate_preliminary_report(case_dir: str | Path, fmt="docx"):
    ...
    run = title.add_run("LAUDO TÉCNICO PERICIAL PRELIMINAR"); run.bold = True; run.font.size = Pt(16)
    sub = doc.add_paragraph(); sub.alignment = WD_ALIGN_PARAGRAPH.CENTER; sub.add_run(case["id"]).bold = True
    ...
    headings = [
        "1. IDENTIFICAÇÃO E OBJETO DA PERÍCIA", "2. MATERIAL RECEBIDO",
        "3. CADEIA DE CUSTÓDIA E PRESERVAÇÃO", "4. METODOLOGIA",
        "5. RESULTADOS E DISCUSSÃO", "6. PROTOCOLO DE MÍDIA SINTÉTICA / DEEPFAKE",
        "7. CONCLUSÃO PRELIMINAR", "8. RESPOSTAS AOS QUESITOS",
        "9. LIMITAÇÕES", "10. REFERÊNCIAS",
    ]
    for h in headings:
        ...
        if h.startswith("1."):
            ...
            for r in reports:
                ...
                table.rows[0].cells[0].text = "Método"; table.rows[0].cells[1].text = "Resultado resumido"
                _repeat_header_row(table.rows[0]); _cant_split_row(table.rows[0])
                for k, v in r.get("methods", {}).items():
                    row = table.add_row(); _cant_split_row(row); c = row.cells
                    c[0].text = METHOD_LABELS.get(k, k)
                    c[1].text = _short_result(k, v)
        elif h.startswith("6."):
            doc.add_paragraph(
                "O protocolo MFLAB-DF separa triagem heurística de evidência produzida por modelos externos validados. Nenhum 'AI score' é convertido automaticamente em veredito. O objetivo é registrar famílias de evidência independentes e encaminhar casos sinalizados para revisão manual e, quando configurado, modelos aprendidos avaliados no domínio de interesse."
            )
            for r in reports:
                ds = _deepfake_summary(r)
                if ds:
            ...
            for q in case.get("questions", []):
                p = doc.add_paragraph(); p.add_run("Quesito: ").bold = True; p.add_run(q)
                p = doc.add_paragraph(); p.add_run("Resposta: ").bold = True; p.add_run(_answer_question(q, reports, assessment))
        elif h.startswith("9."):
            doc.add_paragraph(
