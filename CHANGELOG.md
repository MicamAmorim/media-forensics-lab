...
- CI matrix across Windows/Ubuntu and Python 3.10/3.11/3.12 with a stable `ci-gate` status.
- Canonical ground-truth contract: every demo fixture declares its required validation assertions and CI fails unless GT fixture/assertion coverage and pass rates are all exactly 100%.
- Packaging smoke test, semantic-version consistency gate and tagged GitHub Release pipeline.
- Dependabot configuration for Python and GitHub Actions dependencies.

### Changed
- `dataset/demo/ground_truth.json` is now an independent, versioned oracle and is never silently regenerated by the fixture builder.
- Runtime package version is resolved from the installed distribution/source metadata instead of a hard-coded pipeline version.
...
- 100% GT regression means complete agreement with the controlled test oracle; it is not a sensitivity/specificity estimate or proof of forensic validity in the wild.
